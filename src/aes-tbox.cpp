...
alignas(64) const uint8_t TBox01[256] = {
...
// const uint8_t TBox02[256]
// const uint8_t TBox03[256]
//  注意，TBox0/1/2/3可以有tools/gen_TBox.cpp生成
//  //TBox0[i]=TBox02[i]<<24|TBox01[i]<<16|TBox01[i]<<8|TBox03[i];
//  //TBox1[i]=TBox03[i]<<24|TBox02[i]<<16|TBox01[i]<<8|TBox01[i];
//  //TBox2[i]=TBox01[i]<<24|TBox03[i]<<16|TBox02[i]<<8|TBox01[i];
//  //TBox3[i]=TBox01[i]<<24|TBox01[i]<<16|TBox03[i]<<8|TBox02[i];
alignas(64) const uint32_t TBox0[256] = {
...
alignas(64) const uint32_t TBox1[256] = {
...
alignas(64) const uint32_t TBox2[256] = {
...
alignas(64) const uint32_t TBox3[256] = {
...
alignas(64) const uint32_t Rcon[10] = {0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 0x20000000, 0x40000000, 0x80000000, 0x1B000000, 0x36000000};

static std::array<uint32_t, roundkey_size-8> mid_round_key;
static std::array<uint8_t, 16> white_key;
static std::array<uint8_t, 16> last_key;
// 密钥拓展
void AES_key_expansion(const std::array<uint8_t, 16> &key)
{
    // 密钥拓展
    std::array<uint32_t, roundkey_size> round_key;
    round_key[0] = (key[0] << 24) | (key[1] << 16) | (key[2] << 8) | key[3];
    round_key[1] = (key[4] << 24) | (key[5] << 16) | (key[6] << 8) | key[7];
    round_key[2] = (key[8] << 24) | (key[9] << 16) | (key[10] << 8) | key[11];
    round_key[3] = (key[12] << 24) | (key[13] << 16) | (key[14] << 8) | key[15];
    int i;
    // uint32_t temp;
    // for (i = 4; i < roundkey_size; i++)
    // {
    //     temp = round_key[i - 1];
    //     if (i & 3 == 0)
    //     {
    //         temp = (TBox01[(temp >> 16) & 0xFF] << 24) | (TBox01[(temp >> 8) & 0xFF] << 16) | (TBox01[temp & 0xFF] << 8) | TBox01[temp >> 24];
    //         temp ^= (Rcon[i / 4 - 1]);
    //     }
    //     round_key[i] = round_key[i - 4] ^ temp;
    // }
    for (i = 4; i < roundkey_size; i+=4)
    {
        //temp = round_key[i-1];
        round_key[i] = round_key[i-4] ^ Rcon[i/4-1] ^ ((TBox01[(round_key[i-1] >> 16) & 0xFF] << 24) | (TBox01[(round_key[i-1] >> 8) & 0xFF] << 16) | (TBox01[round_key[i-1] & 0xFF] << 8) | TBox01[round_key[i-1] >> 24]);
    ...
    white_key = key;
    last_key = {
        static_cast<uint8_t>(round_key[roundkey_size-3] >> 24), static_cast<uint8_t>((round_key[roundkey_size-3] >> 16) & 0xFF),
        static_cast<uint8_t>((round_key[roundkey_size-3] >> 8) & 0xFF), static_cast<uint8_t>(round_key[roundkey_size-3] & 0xFF),
        static_cast<uint8_t>(round_key[roundkey_size-2] >> 24), static_cast<uint8_t>((round_key[roundkey_size-2] >> 16) & 0xFF),
        static_cast<uint8_t>((round_key[roundkey_size-2] >> 8) & 0xFF), static_cast<uint8_t>(round_key[roundkey_size-2] & 0xFF),
        static_cast<uint8_t>(round_key[roundkey_size-1] >> 24), static_cast<uint8_t>((round_key[roundkey_size-1] >> 16) & 0xFF),
        static_cast<uint8_t>((round_key[roundkey_size-1] >> 8) & 0xFF), static_cast<uint8_t>(round_key[roundkey_size-1] & 0xFF),
        static_cast<uint8_t>(round_key[roundkey_size] >> 24), static_cast<uint8_t>((round_key[roundkey_size] >> 16) & 0xFF),
        static_cast<uint8_t>((round_key[roundkey_size] >> 8) & 0xFF), static_cast<uint8_t>(round_key[roundkey_size] & 0xFF)};
    std::copy(round_key.begin() + 4, round_key.begin() + 4*Nr, mid_round_key.begin()); // 左闭右开
}
// AES 轮，使用大查表优化法
void AES_round(std::array<uint8_t, 16> &x)
{
    ...
    x[15] ^= white_key[15];
    // 常规轮
    std::array<uint32_t, 4> x32 = {0};
    int i;
    for (i = 0; i < Nr_2; i++)
    {
        x32[0] = TBox0[x[0]] ^ TBox1[x[5]] ^ TBox2[x[10]] ^ TBox3[x[15]] ^ mid_round_key[4 * i];
        x32[1] = TBox0[x[4]] ^ TBox1[x[9]] ^ TBox2[x[14]] ^ TBox3[x[3]] ^ mid_round_key[4 * i + 1];
        x32[2] = TBox0[x[8]] ^ TBox1[x[13]] ^ TBox2[x[2]] ^ TBox3[x[7]] ^ mid_round_key[4 * i + 2];
        x32[3] = TBox0[x[12]] ^ TBox1[x[1]] ^ TBox2[x[6]] ^ TBox3[x[11]] ^ mid_round_key[4 * i + 3];
        x[0] = (x32[0] >> 24);
        x[1] = (x32[0] >> 16) & 0xFF;
        x[2] = (x32[0] >> 8) & 0xFF;
        x[3] = x32[0] & 0xFF;
    ...
    x32[0] = TBox0[x[0]] ^ TBox1[x[5]] ^ TBox2[x[10]] ^ TBox3[x[15]] ^ mid_round_key[4 * Nr_2];
    x32[1] = TBox0[x[4]] ^ TBox1[x[9]] ^ TBox2[x[14]] ^ TBox3[x[3]] ^ mid_round_key[4 * Nr_2 + 1];
    x32[2] = TBox0[x[8]] ^ TBox1[x[13]] ^ TBox2[x[2]] ^ TBox3[x[7]] ^ mid_round_key[4 * Nr_2 + 2];
    x32[3] = TBox0[x[12]] ^ TBox1[x[1]] ^ TBox2[x[6]] ^ TBox3[x[11]] ^ mid_round_key[4 * Nr_2 + 3];
    // 最后一轮
    x[0] = TBox01[(x32[0] >> 24)] ^ last_key[0];
    x[1] = TBox01[(x32[1] >> 16) & 0xFF] ^ last_key[1];
    x[2] = TBox01[(x32[2] >> 8) & 0xFF] ^ last_key[2];
    x[3] = TBox01[x32[3] & 0xFF] ^ last_key[3];
    x[4] = TBox01[(x32[1] >> 24)] ^ last_key[4];
    x[5] = TBox01[(x32[2] >> 16) & 0xFF] ^ last_key[5];
    x[6] = TBox01[(x32[3] >> 8) & 0xFF] ^ last_key[6];
    x[7] = TBox01[x32[0] & 0xFF] ^ last_key[7];
    x[8] = TBox01[(x32[2] >> 24)] ^ last_key[8];
    x[9] = TBox01[(x32[3] >> 16) & 0xFF] ^ last_key[9];
    x[10] = TBox01[(x32[0] >> 8) & 0xFF] ^ last_key[10];
    x[11] = TBox01[x32[1] & 0xFF] ^ last_key[11];
    x[12] = TBox01[(x32[3] >> 24)] ^ last_key[12];
    x[13] = TBox01[(x32[0] >> 16) & 0xFF] ^ last_key[13];
    x[14] = TBox01[(x32[1] >> 8) & 0xFF] ^ last_key[14];
    x[15] = TBox01[x32[2] & 0xFF] ^ last_key[15];
}

/*
g++ -std=c++17 AES_Tbox.cpp -o AES_Tbox
./AES_Tbox
*/